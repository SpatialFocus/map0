...
const KEY_TABLES: Array<[definition: string, keys: readonly string[]]> = [
  ...
  ["wfsLayer", LAYER_KEYS.wfs!],
  ["ogcapiFeaturesLayer", LAYER_KEYS["ogcapi-features"]!],
...
const ENUMS: Array<[path: string, values: readonly unknown[]]> = [
  ...
  ["wfsLayer.properties.version.enum", WFS_VERSIONS],
...
describe("v1.json required keys", () => {
  ...
  it.each([
    ...
    ["wfsLayer", ["type", "url", "typeNames"]],
    ["ogcapiFeaturesLayer", ["type", "url"]],
